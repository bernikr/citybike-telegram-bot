...
def get_nearby_stations(update, context):
    if update.message:
        ...
        if 'home_station' not in context.user_data:
            ...
            home_station = [s for s in stations if s['id']==context.user_data['home_station']][0]
...
def set_home(update, context):
    context.bot.send_message(chat_id=update.message.chat_id, text="Send a location to set its nearest station as home.\nUse /cancel to cancel, or /delete to remove it")
    return "SETHOME"
...
def set_home_location(update, context):
    ...
    context.user_data['home_station'] = home_station['id']

    context.bot.send_message(chat_id=update.message.chat_id, text="Home Station set to:\n\n" + format_station_text(home_station),parse_mode="Markdown", disable_web_page_preview=True)
...
def delete_home(update, context):
    context.user_data['home_station'] = None
    context.bot.send_message(chat_id=update.message.chat_id, text="Your Home Station was deleted!\nUse /sethome to set it again")
    return -1
